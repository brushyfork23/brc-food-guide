...
const year = "2022"
const campsFilename = "brc_api_" + year + "/camps.json"
const artFilename = "brc_api_" + year + "/art.json"
const eventsFilename = "brc_api_" + year + "/events.json"
...
func main() {
	...
	for decoder.More() {
		...
		for _, occurrence := range event.OccurrenceSet {
			...
			header := fmt.Sprintf("%s   %s   (%s)   %s", formattedEvent.EventName, formattedEvent.LongTimes, formattedEvent.Address, formattedEvent.LocationName)
			if utf8.RuneCountInString(longestHeader) < utf8.RuneCountInString(header) {
	...
	make_title_page(pdf)
	// Intentional blank page on the back side of the title page
	pdf.AddPage()
	pdf.AddPage()
	pdf.SetFont("Arial", "", 8)
...
func make_title_page(pdf *gofpdf.Fpdf) {
	...
	pdf.WriteAligned(0, 38, "BRC "+year, "C")
